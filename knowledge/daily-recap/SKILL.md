...
候选表确认后，**按主题域**扇出 subagent 并行收集（一个主题域一个 subagent，包揽该主题下所有会话）。

> **写权限归属**：主 agent 独占日记写权限。subagent 只返回证据包文本，4b 对齐门和 4d 核验在主 agent 写入前执行，subagent 不得越过。

**扇出参数（照此构造，勿即兴发挥）**：

- `agent: explorer`（只读，工具层无 write/edit/bash，从源头消除写入风险）
- `systemPrompt` 必须含硬约束：「你是证据收集员，只读不写。禁止 write/edit/创建/修改任何文件，禁止写入日记文件。只返回证据包文本。」
- `task` 结构：主题域 + 会话清单（filepath / 时间 CST / 标题）+ 重点核实项。**不传「去向」**--去向是 Step 5 主 agent 写入用的，传给 subagent 会诱导它直接写日记。

每个 subagent 的收集步骤：

1. **读会话**：本机会话用 `ffgrep` 搜 jsonl 内的工具调用路径 + `read`（带 offset/limit）定向读关键行（explorer 无 bash，用这两个工具替代 jq/grep）；远程会话用 `nmem_read_thread`（`thread_id`，`offset: 0` 起续读）读会话线程。
2. **自发现产出物路径**：从会话的工具调用里抽 read/write/edit 涉及的文件路径，筛出关键文档（方案/规格/issue/报告），`read` 它们--不靠会话摘要猜内容。
...
5. **返回证据包文本**：每条含关键结论、带来源的数字、决策、建议的叙事角度。**返回即止，不写文件**。
...
- `agent: reviewer`（只读 + bash 限只读命令，glm-5.2 强模型适合逐条核对数字）
- `systemPrompt` 含硬约束：「你只读。禁止写入/创建/修改任何文件，禁止重定向写入。bash 仅用于只读命令。只报告有证据的不一致项。」
