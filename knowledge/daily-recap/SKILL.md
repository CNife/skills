...
候选表确认后，**按主题域**扇出 subagent 并行收集（一个主题域一个 subagent，包揽该主题下所有会话）。每个 subagent：

1. **读会话**：本机会话用证据集（Step 0 脚本输出）里的 filepath 读原始 jsonl；远程会话用 `nmem_read_thread`（`thread_id`，`offset: 0` 起续读）读会话线程。
2. **自发现产出物路径**：从会话的工具调用里抽 read/write/edit 涉及的文件路径，筛出关键文档（方案/规格/issue/报告），读它们--不靠会话摘要猜内容。
...
5. **返回结构化证据包**：每条含关键结论、带来源的数字、决策、建议的叙事角度。

**降级**：若 subagent 无法访问 nmem 工具，远程会话改由主 agent 串行 `nmem_read_thread`，混合主题（本机+远程）的综合也落回主 agent。
单会话读取失败标记「内容待补充」，不阻塞该主题域。

**完成条件**：每个批准的主题域都有一份证据包，每条关键结论/数字有原文或原文档出处。
...
- 核验 subagent 报告不一致项（数字错配、口径混淆、最高/最低写反等）。
- **发现不一致 -> HITL**：报告给你，你拍板怎么改；改完再写入。无不一致则直接进入 Step 5。
