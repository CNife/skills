...
日记文件使用 **YYYY/MM/ 双层子目录** 组织，**不是**直接放在日记根目录下。
...
写入之后、确认之前，必须做一次风格自检。**以下三项任何一项不通过，都应修改后再确认：**

#### ① 语气检查

读一遍刚写的内容，问自己：

- 这段读起来像工作周报吗？（"完成了 X → 新建了 Y → 更新了 Z"的列表节奏）
- 有人称和感受吗？（"觉得"、"没想到"、"比想象中好"、"终于"这类词）
- 有"为什么"吗？（为什么选 A 不选 B，而不是只记"选了 A"）
- 如果三个都是否 → **这是流水账，重写为叙事风格**

参考 `references/personal-diary.md` 的「语气」对比示例：保留个人感受，不要工作报告调子。

#### ② 结构检查

检查日记结构：

- 是否按主题域聚合，而不是按对话时间顺序平铺？（一个 `#` 标题对应一个主题域，不是一条操作一个标题）
- 新内容是否塞进了今日日记已有的对应章节？（步骤 2c 的要求）
- `#` 标题数量是否合理？（如果一写就是 5+ 个一级标题，可能是流水账信号）
- 如果以上任何一项不通过 → **重新聚合主题再写**

#### ③ 噪音检查

对照「AI 噪音过滤」表（见下文），确认没有写入以下内容：
...
**发现噪音 → 删掉，只保留人看到的结论和决策。**

#### 自检通过后

如果三项都通过，或者按要求修正后通过 → 继续到步骤 4 确认。
