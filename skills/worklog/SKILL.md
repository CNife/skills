...
```bash
uv run ~/personal_code/skills/skills/worklog/scripts/extract.py --since today
```
...
开始分类前先判当日日期：
- 周末 → `day_context = holiday`
- 中国法定假期 → `day_context = holiday`
- 工作日 → `day_context = workday`

法定假期参考：元旦(1/1)、春节(除夕~初六)、清明(4/4~4/6)、五一(5/1~5/5)、端午、中秋、国庆(10/1~10/7)。

### Step 2b：分类（两条信号，按顺序判定）

**信号 1 — 主题活动词所在的 vault**

对每条活动提取核心主题词，查用户的两个 Obsidian vault：

```bash
# 查工作 vault
grep -rli "$主题词" /mnt/c/Obsidian/工作/ --include='*.md' 2>/dev/null | head -3
# 查个人 vault
grep -rli "$主题词" /mnt/c/Obsidian/个人/ --include='*.md' 2>/dev/null | head -3
```

| 结果 | 归属 |
|------|------|
| 主题词仅在工作 vault 有命中 | **工作** |
| 主题词仅在个人 vault 有命中 | **个人** |
| 两个 vault 都有命中或都无命中 | 走信号 2 |

> **示例：**「FRP TLS 加密配置」→ 搜 `FRP`，个人 vault 有笔记 → 个人
> 「OneReason 数据库修复」→ 搜 `OneReason`，工作 vault 有笔记 → 工作

**信号 2 — 日期上下文兜底**

- `holiday` 模式 → **个人**（所有模糊项归个人）
- `workday` 模式 → **工作**（所有模糊项归工作）
...
- 结尾的待完成列表放在工作日志末尾
- Holiday 模式：工作日志只写一句话（如「五一假期，无工作活动」），不回顾前一天工作、不列待办、不写节后计划，工作线点评跳过
...
| Worklog 脚本 | `~/personal_code/skills/skills/worklog/scripts/extract.py` |
