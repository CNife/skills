...
- Source state exists and is a git repo: `chezmoi source-path` and `chezmoi git -- rev-parse --git-dir` inside it
- Remote `origin` configured: `chezmoi git -- remote get-url origin`
- Remote HEAD branch: `main` (assumed; verify with `chezmoi git -- symbolic-ref refs/remotes/origin/HEAD`)
...
head_before=$(chezmoi git -- rev-parse HEAD)
...
behind_remote=$(chezmoi git -- log --oneline HEAD..origin/main 2>/dev/null)
ahead_remote=$(chezmoi git -- log --oneline origin/main..HEAD 2>/dev/null)
...
echo "---"
echo "当前状态一览:"
echo "  本地 HEAD: $(chezmoi git -- rev-parse --short HEAD)"
echo "  远程:      $(chezmoi git -- rev-parse --short origin/main 2>/dev/null || echo 'N/A')"
```
...
remote_new=$(chezmoi git -- log --oneline HEAD..origin/main)
...
If `chezmoi git -- pull --autostash --rebase` fails due to conflicts, perform a
...
for file in $conflicted_files; do
  ...
  if [ "$remote_commit_date" -gt "$local_commit_date" ] && [ "$days_since_local_commit" -gt 7 ]; then
    ...
    chezmoi git -- checkout --theirs -- "$file"
    ...
    chezmoi git -- checkout --ours -- "$file"
    chezmoi git -- add "$file"
...
if [ -n "$needs_user" ]; then
  ...
  echo "  1) 采用本地版本 (chezmoi git checkout --ours)"
  echo "  2) 采用远程版本 (chezmoi git checkout --theirs)"
  echo "  3) 手动编辑后告诉我继续"
  echo ""
  echo "→ 请告诉我每个文件的处理方式。"
...
if [ -n "$stash_conflicts" ]; then
  ...
  for f in $stash_conflicts; do
    echo "  - $f (自动采用拉取后版本)"
    chezmoi git -- checkout --theirs -- "$f" 2>/dev/null && chezmoi git -- add "$f"
  done
  echo "✅ 存储冲突已解决"
fi
```
...
final_changes=$(chezmoi git -- status --porcelain)
if [ -z "$final_changes" ]; then
  echo "✅ 同步完成，本地没有未提交变更"
  echo "  本地:  $(chezmoi git -- rev-parse --short HEAD)"
  echo "  远程:  $(chezmoi git -- rev-parse --short origin/main)"
  echo ""
  echo "最近提交:"
...
3. **All git operations** use `chezmoi git -- <cmd>` — never hardcode the source directory path
